...
class MainMenu(object):
    def __init__(self,screen,x,y,width,height):
        ...
        self.music_screen_menu = pygame.image.load("music option screen/pixil-frame-0.png")
        self.music_off_screen = pygame.image.load("music option screen/pixil-frame-1.png")
        self.character_info_screen = pygame.image.load("character_option_brawler.png")
        #loading music
        mixer.music.load("sound folder/main_background_music_brawler.mp3")
    ...
    def music_screen(self,events):
        ...
        for event in events:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                self.mouse_x, self.mouse_y = pygame.mouse.get_pos()
                if 380 <= self.mouse_x <= 600 and 130 <= self.mouse_y <= 200:  # Corrected logic
                    if self.is_music == True:
                        self.is_music = False
                    elif self.is_music == False:
                        self.is_music = True
        ...
        self.screen.fill(self.blank_state)
        if self.is_music == True:
            self.screen.blit(self.music_screen_menu,(self.x,self.y))
        elif self.is_music == False:
            self.screen.blit(self.music_off_screen,(self.x,self.y))
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_LCTRL:
                    ...
                    self.character_info_selected = False 
        self.screen.fill(self.blank_state)
        self.screen.blit(self.character_info_screen,(self.x,self.y))
        pass
